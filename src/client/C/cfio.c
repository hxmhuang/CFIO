...
int cfio_init(int x_proc_num, int y_proc_num, int ratio)
{
    ...
    send_buf[0] = send_buf[1] = send_buf[2] = send_buf[3] = 9.0;
    double time1, time2, time3, time4;
    MPI_Status sta;
    if(cfio_map_proc_type(rank) == CFIO_MAP_TYPE_CLIENT) {
	int server_id = cfio_map_get_server_of_client(rank);
	MPI_Ssend(send_buf, 4, MPI_DOUBLE, server_id, 0, MPI_COMM_WORLD);
	time1 = times_cur();
	MPI_Recv(recv_buf, 4, MPI_DOUBLE, server_id, 1, MPI_COMM_WORLD, &sta);
	time2 = times_cur();
	MPI_Ssend(send_buf, 4, MPI_DOUBLE, server_id, 2, MPI_COMM_WORLD);
	time3 = times_cur();
	MPI_Recv(recv_buf, 4, MPI_DOUBLE, server_id, 3, MPI_COMM_WORLD, &sta);
	time4 = times_cur();

	double x = (time3 - time2 + recv_buf[1] - recv_buf[0]) / 2; 
	double y2x = (time2 - time1 + time4 - time3 + recv_buf[2] - recv_buf[1]) / 3;
	double y = (y2x - x )/ 2;
	base_time = time3 + y - recv_buf[2] + recv_buf[3];

    }else if(cfio_map_proc_type(rank) == CFIO_MAP_TYPE_SERVER) {
	int client_num = cfio_map_get_client_num_of_server(rank);
	int *client_ids = malloc(sizeof(int) * client_num);
	cfio_map_get_clients(rank, client_ids);
	send_buf[3] = base_time;
	for (i = 0; i < client_num; ++i) {
	    MPI_Recv(recv_buf, 4, MPI_DOUBLE, client_ids[i], 0, MPI_COMM_WORLD, &sta);
	    send_buf[0] = times_cur(); // time1
	    MPI_Ssend(send_buf, 4, MPI_DOUBLE, client_ids[i], 1, MPI_COMM_WORLD);
	    send_buf[1] = times_cur(); // time2
	    MPI_Recv(recv_buf, 4, MPI_DOUBLE, client_ids[i], 2, MPI_COMM_WORLD, &sta);
	    send_buf[2] = times_cur(); // time3
	    MPI_Ssend(send_buf, 4, MPI_DOUBLE, client_ids[i], 3, MPI_COMM_WORLD);
	}
	free(client_ids);
    }
    printf("event-driven proc %d base_time %f \n", rank, base_time);
#endif // event-driven
	...
	printf("server %d cfio_server_start time %f. \n", rank, server_start_end - server_start_begin);
