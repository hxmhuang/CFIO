...
static int merge_send_num = 0;
static int msg_order = 1;

#ifdef TEST_ED
static int itr = 1;
static double ready_time;
...
static inline void _main_send_msg(cfio_msg_t *msg)
{
	...
	printf("proc %d begin test-ed to server %d \n", rank, msg->src);
	char test_msg[40];
	memset(test_msg, 'z', 40);
	ready_time = times_cur() - base_time;
	MPI_Ssend(test_msg, 40, MPI_CHAR, msg->dst, rank, MPI_COMM_WORLD);
	finish_time = times_cur() - base_time;
...
//	    msg->dst, tag, msg->comm, &(msg->req));
	...
	// zc
	if (0 == rank)
	    printf("%f: client 0 send_num %d, merge_send_num %d \n", times_cur(), send_num, merge_send_num);
	// zc
...
#ifdef TEST_ED // event-driven
    printf("event-driven proc %d ready_time %f finish_time %f \n", rank, ready_time, finish_time);
    int server = cfio_map_get_server_of_client(rank);
    // MPI_Send(&base_time, 1, MPI_DOUBLE, server, 0, MPI_COMM_WORLD);
    MPI_Send(&ready_time, 1, MPI_DOUBLE, server, 1, MPI_COMM_WORLD);
    MPI_Send(&finish_time, 1, MPI_DOUBLE, server, 2, MPI_COMM_WORLD);
#endif // event-driven
